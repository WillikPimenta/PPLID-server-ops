...
FILA_CONTEXTO_REINSPECAO = ReinspecaoAuditorPresence.CONTEXTO_REINSPECAO
FILA_CONTEXTO_AUDITORIA_COMPLIANCE = ReinspecaoAuditorPresence.CONTEXTO_AUDITORIA_COMPLIANCE
FILA_CONTEXTOS = {FILA_CONTEXTO_REINSPECAO, FILA_CONTEXTO_AUDITORIA_COMPLIANCE}
_fila_contexto: ContextVar[str] = ContextVar(
    "qualidade_fila_contexto",
    default=FILA_CONTEXTO_REINSPECAO,
)

ANALISE_ATIVA = (
    QualidadePendenteReinspecao.ANALISE_AGUARDANDO,
    QualidadePendenteReinspecao.ANALISE_EM_ANALISE,
)
...
ANALISE_LABELS = {
    **dict(QualidadePendenteReinspecao.ANALISE_STATUS_CHOICES),
    QualidadePendenteReinspecao.ANALISE_CONCLUIDO: "Concluído",
    QualidadePendenteReinspecao.ANALISE_NAO_ATRIBUIDO: "Não atribuído",
}
PRESENCE_LABELS = dict(ReinspecaoAuditorPresence.STATUS_CHOICES)


def normalize_fila_contexto(value: str | None) -> str:
    contexto = (value or "").strip().lower()
    return contexto if contexto in FILA_CONTEXTOS else FILA_CONTEXTO_REINSPECAO


def set_fila_contexto(value: str | None) -> Token:
    return _fila_contexto.set(normalize_fila_contexto(value))


def reset_fila_contexto(token: Token) -> None:
    _fila_contexto.reset(token)


def fila_contexto_atual() -> str:
    return _fila_contexto.get()
...
def get_or_create_presence(user) -> ReinspecaoAuditorPresence:
    presence, _ = ReinspecaoAuditorPresence.objects.get_or_create(
        user=user,
        contexto=fila_contexto_atual(),
    )
    return presence
...
def _presence_qs() -> QuerySet[ReinspecaoAuditorPresence]:
    return ReinspecaoAuditorPresence.objects.filter(contexto=fila_contexto_atual())


def reinspecao_falhas_qs() -> QuerySet[QualidadePendenteReinspecao]:
    """Fila/consulta em andamento: protocolos pendentes (P…)."""
    return QualidadePendenteReinspecao.objects.filter(contexto=fila_contexto_atual())
...
def serialize_pendente_reinspecao(pendente: QualidadePendenteReinspecao, *, agente_nome: str | None = None) -> dict:
    origem = (pendente.fila_origem or "").strip() or QualidadePendenteReinspecao.FILA_ORIGEM_AUTO
    analise_status = pendente.analise_status or QualidadePendenteReinspecao.ANALISE_NAO_ATRIBUIDO
    return {
        "id": pendente.id,
        ...
        "fila_origem_label": (
            "Direcionado"
            if origem == QualidadePendenteReinspecao.FILA_ORIGEM_DIRECIONADO
            else "Automático"
        ),
...
def _timeout_excluded_protocolos_for_auditor(user) -> set[str]:
    ...
    rows = (
        ReinspecaoFilaHistorico.objects.filter(
            contexto=fila_contexto_atual(),
            tipo=ReinspecaoFilaHistorico.TIPO_LIBERACAO_TIMEOUT,
            auditor_id=user.id,
...
def _pick_auditor(online: list[ReinspecaoAuditorPresence], loads: dict):
    def sort_key(presence: ReinspecaoAuditorPresence):
    ...
    *,
    ...
    if not origem:
        if tipo_historico in {
            ReinspecaoFilaHistorico.TIPO_DIRECIONAMENTO,
            ReinspecaoFilaHistorico.TIPO_REATRIBUICAO,
        }:
            origem = QualidadePendenteReinspecao.FILA_ORIGEM_DIRECIONADO
        else:
            origem = QualidadePendenteReinspecao.FILA_ORIGEM_AUTO
    falha.responsavel = auditor
    falha.analise_status = QualidadePendenteReinspecao.ANALISE_AGUARDANDO
...
def _assign_protocolo_group(
    ...
    *,
    ...
    siblings = list(
        ...
        .exclude(analise_status=QualidadePendenteReinspecao.ANALISE_CONCLUIDO)
...
def _release_falha(
    ...
    *,
    ...
    falha.analise_status = QualidadePendenteReinspecao.ANALISE_NAO_ATRIBUIDO
...
def release_auditor_active_queue(*, auditor, actor, motivo: str) -> int:
    ...
    for falha in falhas:
        ...
        _release_falha(
            falha,
            actor=actor,
            tipo_historico=ReinspecaoFilaHistorico.TIPO_LIBERACAO_STATUS,
            motivo=motivo,
        )
    return released
...
def remover_protocolo_direcionado(*, falha_id: int, user) -> dict[str, Any]:
    ...
    if falha.id not in waiting_ids or origem != QualidadePendenteReinspecao.FILA_ORIGEM_DIRECIONADO:
...
def expire_stale_fila_assignments(*, actor=None) -> int:
    ...
    for falha in falhas:
        ...
        _release_falha(
            falha,
            actor=actor,
            tipo_historico=ReinspecaoFilaHistorico.TIPO_LIBERACAO_TIMEOUT,
            motivo="timeout_fila",
        )
        if protocolo not in seen:
            seen.add(protocolo)
    ...
    for auditor_id in affected_auditor_ids:
        ...
        if presence and presence.status == ReinspecaoAuditorPresence.STATUS_ONLINE:
            set_auditor_status(
                user=auditor,
                status=ReinspecaoAuditorPresence.STATUS_OFFLINE,
                actor=actor,
            )
...
@transaction.atomic
def set_auditor_status(*, user, status: str, actor=None) -> ReinspecaoAuditorPresence:
    status = (status or "").strip().lower()
    valid = {c[0] for c in ReinspecaoAuditorPresence.STATUS_CHOICES}
    ...
    if not presence:
        presence = ReinspecaoAuditorPresence(
            user=user,
            contexto=fila_contexto_atual(),
        )

    previous = presence.status if presence.pk else ReinspecaoAuditorPresence.STATUS_OFFLINE
    if (
        status == ReinspecaoAuditorPresence.STATUS_ONLINE
        and previous != ReinspecaoAuditorPresence.STATUS_ONLINE
    ):
        assert_can_go_online(user=user, target_context=FILA_CONTEXTO_REINSPECAO)
    now = timezone.now()
    ...
    _write_historico(
        tipo=ReinspecaoFilaHistorico.TIPO_STATUS_AUDITOR,
        actor=actor,
        auditor=user,
    ...
    if status in {
        ReinspecaoAuditorPresence.STATUS_OFFLINE,
        ReinspecaoAuditorPresence.STATUS_AUSENTE,
    }:
...
def set_auditor_status_and_maybe_distribute(*, user, status: str, actor=None) -> dict:
    ...
    distribution = None
    if presence.status == ReinspecaoAuditorPresence.STATUS_ONLINE:
...
def distribuir_protocolos(*, actor=None, limit: int | None = None) -> dict[str, Any]:
    ...
    online = list(
        _presence_qs().select_for_update()
        .filter(status=ReinspecaoAuditorPresence.STATUS_ONLINE)
        .select_related("user")
        .order_by("id")
    )
    if not online:
    ...
    for seed in seeds:
        ...
        _assign_protocolo_group(
            ...
            auditor=auditor,
            actor=actor,
            tipo_historico=ReinspecaoFilaHistorico.TIPO_ATRIBUICAO_AUTO,
            fila_origem=QualidadePendenteReinspecao.FILA_ORIGEM_AUTO,
        )
...
def claim_next_for_auditor(*, user) -> AuditoriaFalhaCadastro | None:
    ...
    if not presence or presence.status != ReinspecaoAuditorPresence.STATUS_ONLINE:
        return None
...
def direcionar_protocolo(
    *,
    ...
    if falha.analise_status == QualidadePendenteReinspecao.ANALISE_CONCLUIDO:
    ...
    tipo = (
        ...
        else ReinspecaoFilaHistorico.TIPO_DIRECIONAMENTO
    )
    return _assign_protocolo_group(
        falha,
        auditor=auditor,
        ...
        fila_origem=QualidadePendenteReinspecao.FILA_ORIGEM_DIRECIONADO,
        start_sla=start_sla,
    )
...
def iniciar_analise(*, falha_id: int, user, allow_any: bool = False) -> AuditoriaFalhaCadastro:
    ...
    now = timezone.now()
    restarted = falha.analise_status == QualidadePendenteReinspecao.ANALISE_EM_ANALISE
    if falha.analise_status == QualidadePendenteReinspecao.ANALISE_AGUARDANDO:
        falha.analise_status = QualidadePendenteReinspecao.ANALISE_EM_ANALISE
        falha.analise_iniciada_em = now
    elif restarted:
        # Reinicia o timeout sem alterar o início original da análise.
        pass
    ...
    _write_historico(
        tipo=ReinspecaoFilaHistorico.TIPO_INICIO_ANALISE,
        actor=user,
        pendente=falha,
...
def _ensure_reinspecao_mapping(falha, resolver) -> None:
    if resolver is None or falha.mapping_version:
        return
    from apps.auditoria.services.reinspecao_mapping import status_for_existing

    resolution = resolver.resolve(falha.descricao_irregularidades)
    existing_scenario = falha.motivo_falha
    existing_stage = falha.etapa_falha
    falha.codigo_irregularidade = resolution.extracted.code
    if not (existing_scenario or "").strip() and resolution.scenario:
        falha.motivo_falha = resolution.scenario
    if not (existing_stage or "").strip() and resolution.stage:
        falha.etapa_falha = resolution.stage
    falha.mapping_scenario_status = status_for_existing(
        existing_scenario,
        resolution.scenario,
        resolution.scenario_status,
    )
    falha.mapping_stage_status = status_for_existing(
        existing_stage,
        resolution.stage,
        resolution.stage_status,
    )
    falha.mapping_version = resolution.mapping_version
    falha.mapping_source_hash = resolution.source_hash
    falha.mapping_applied_at = timezone.now()
    parsed = dict(falha.brflow_parsed or {}) if isinstance(falha.brflow_parsed, dict) else {}
    parsed["reinspecao_mapping"] = {
...
def _apply_etapa_fields(falha: AuditoriaFalhaCadastro, etapa: dict) -> str:
    ...
    incoming_stage = str(etapa.get("etapa_falha") or "").strip()
    incoming_scenario = str(etapa.get("motivo_falha") or "").strip()
    mapping_conflicts = []
    if falha.mapping_version:
        if not (falha.etapa_falha or "").strip() and incoming_stage:
            falha.etapa_falha = incoming_stage
            falha.mapping_stage_status = falha.MAPPING_PREEXISTING
        elif incoming_stage and incoming_stage != (falha.etapa_falha or "").strip():
            falha.mapping_stage_status = falha.MAPPING_CONFLICT
            mapping_conflicts.append("etapa_falha")
        if not (falha.motivo_falha or "").strip() and incoming_scenario:
            falha.motivo_falha = incoming_scenario
            falha.mapping_scenario_status = falha.MAPPING_PREEXISTING
        elif incoming_scenario and incoming_scenario != (falha.motivo_falha or "").strip():
            falha.mapping_scenario_status = falha.MAPPING_CONFLICT
            mapping_conflicts.append("motivo_falha")
    else:
        falha.etapa_falha = incoming_stage
        falha.motivo_falha = incoming_scenario
    ...
    if mapping_conflicts:
        parsed["reinspecao_mapping_input_conflicts"] = mapping_conflicts
    falha.brflow_parsed = parsed
    return status_norm
...
def concluir_analise(
    *,
    ...
    if fila_contexto_atual() == FILA_CONTEXTO_REINSPECAO:
        from apps.auditoria.services.reinspecao_mapping import ReinspecaoMappingResolver

        mapping_resolver = ReinspecaoMappingResolver.from_active_mapping()
    ...
    if etapas_list:
        ...
        for index, etapa in enumerate(etapas_list):
            ...
            if target is None:
                if fila_contexto_atual() == FILA_CONTEXTO_AUDITORIA_COMPLIANCE:
                    ...
                    target = QualidadePendenteReinspecao(
                        protocolo=falha.protocolo,
            ...
            _ensure_reinspecao_mapping(target, mapping_resolver)
            ...
            target.analise_status = QualidadePendenteReinspecao.ANALISE_CONCLUIDO
        ...
        for leftover in siblings[len(etapas_list) :]:
            if leftover.analise_status == QualidadePendenteReinspecao.ANALISE_CONCLUIDO:
                continue
            if not leftover.analise_iniciada_em:
                leftover.analise_iniciada_em = now
            leftover.analise_status = QualidadePendenteReinspecao.ANALISE_CONCLUIDO
...
def serialize_presence(presence: ReinspecaoAuditorPresence) -> dict:
    return {
...
def serialize_fila_item(
    ...
    *,
    ...
    sla_segundos = None
    if falha.analise_iniciada_em and (falha.analise_status or "") == QualidadePendenteReinspecao.ANALISE_EM_ANALISE:
    ...
    origem = (falha.fila_origem or "").strip() or QualidadePendenteReinspecao.FILA_ORIGEM_AUTO
    ...
    return {
        ...
        "codigo_irregularidade": falha.codigo_irregularidade or "",
        "analise_status": falha.analise_status or QualidadePendenteReinspecao.ANALISE_NAO_ATRIBUIDO,
        "analise_status_label": ANALISE_LABELS.get(
            falha.analise_status or QualidadePendenteReinspecao.ANALISE_NAO_ATRIBUIDO,
            "Não atribuído",
        ),
        "fila_origem": origem,
        "fila_origem_label": (
        ...
        "mapping_scenario_status": falha.mapping_scenario_status or "",
        "mapping_stage_status": falha.mapping_stage_status or "",
...
def _group_protocolo_slots(falhas: list[AuditoriaFalhaCadastro]) -> list[list[AuditoriaFalhaCadastro]]:
    ...
    def sort_key(proto: str):
        items = groups[proto]
        has_em_analise = any(
            item.analise_status == QualidadePendenteReinspecao.ANALISE_EM_ANALISE for item in items
        )
        atribuido = min(
        ...
        slots.append(
            {
                ...
                "fila_origem": (
                    (primary.fila_origem or "").strip() or QualidadePendenteReinspecao.FILA_ORIGEM_AUTO
                ),
...
def list_historico(*, falha_id: int | None = None, auditor_id=None, limit: int = 100) -> list[dict]:
    qs = (
        ReinspecaoFilaHistorico.objects.filter(contexto=fila_contexto_atual())
        .select_related("falha", "pendente", "auditor", "actor")
        .order_by("-created_at", "-id")
    )
    if falha_id:
        # A fila ativa usa QualidadePendenteReinspecao; registros já promovidos
        ...
        results.append(
            {
                "id": item.id,
                "tipo": item.tipo,
                "tipo_label": dict(ReinspecaoFilaHistorico.TIPO_CHOICES).get(item.tipo, item.tipo),
...
def list_auditores_controle() -> list[dict]:
        ...
        for row in (
            ...
            .annotate(
                aguardando=Count(
                    "protocolo",
                    distinct=True,
                    filter=Q(analise_status=QualidadePendenteReinspecao.ANALISE_AGUARDANDO),
                ),
                em_analise=Count(
                    "protocolo",
                    distinct=True,
                    filter=Q(analise_status=QualidadePendenteReinspecao.ANALISE_EM_ANALISE),
                ),
                concluidos=Count(
                    "protocolo",
                    distinct=True,
                    filter=Q(analise_status=QualidadePendenteReinspecao.ANALISE_CONCLUIDO),
                ),
                ativos=Count(
                    "protocolo",
                    distinct=True,
    ...
    for user_id, meta in sorted(users_meta.items(), key=lambda x: str(x[1].get("label") or "")):
        ...
        status = presence.status if presence else ReinspecaoAuditorPresence.STATUS_OFFLINE
...
def _online_seconds_by_auditor(
    *,
    ...
    for uid in ids:
        ...
        if uid in prior_status:
            current = prior_status[uid] or ReinspecaoAuditorPresence.STATUS_OFFLINE
        elif presence and presence.status_changed_at and presence.status_changed_at < start:
            current = presence.status
        else:
            current = ReinspecaoAuditorPresence.STATUS_OFFLINE
        ...
        for ts, to_status in events:
            if ts < cursor:
                current = to_status
                continue
            if current == ReinspecaoAuditorPresence.STATUS_ONLINE:
        ...
        if presence and presence.status == ReinspecaoAuditorPresence.STATUS_ONLINE:
            ...
            elif current == ReinspecaoAuditorPresence.STATUS_ONLINE:
...
def build_controle_operacoes(
    *,
    ...
    presence_counts = {
        "online": sum(1 for a in auditores if a["status"] == ReinspecaoAuditorPresence.STATUS_ONLINE),
        "ausente": sum(1 for a in auditores if a["status"] == ReinspecaoAuditorPresence.STATUS_AUSENTE),
        "offline": sum(1 for a in auditores if a["status"] == ReinspecaoAuditorPresence.STATUS_OFFLINE),
    }
    ...
    from apps.auditoria.services.reinspecao_ged_divergencias import get_reinspecao_ged_ultima_ingestao_em
    from apps.auditoria.services.reinspecao_sla import (
    ...
    return {
        "kpis": {
            ...
            "protocolos_aguardando": int(status_counts.get(QualidadePendenteReinspecao.ANALISE_AGUARDANDO, 0)),
            "protocolos_em_analise": int(status_counts.get(QualidadePendenteReinspecao.ANALISE_EM_ANALISE, 0)),
        ...
        "ultima_ingestao_bot_em": get_reinspecao_ged_ultima_ingestao_em(),
...
def dashboard_reinspecao(*, date_from: datetime | None = None, date_to: datetime | None = None) -> dict:
    ...
    return {
        "auditores": {
            "online": int(presence_counts.get(ReinspecaoAuditorPresence.STATUS_ONLINE, 0)),
            "offline": int(presence_counts.get(ReinspecaoAuditorPresence.STATUS_OFFLINE, 0)),
            "ausente": int(presence_counts.get(ReinspecaoAuditorPresence.STATUS_AUSENTE, 0)),
        },
        "protocolos": {
            "nao_atribuido": int(status_counts.get(QualidadePendenteReinspecao.ANALISE_NAO_ATRIBUIDO, 0)),
            "aguardando_analise": int(status_counts.get(QualidadePendenteReinspecao.ANALISE_AGUARDANDO, 0)),
            "em_analise": int(status_counts.get(QualidadePendenteReinspecao.ANALISE_EM_ANALISE, 0)),
...
def _consulta_column_db_map(use_tratados: bool) -> dict[str, str]:
    if use_tratados:
        return {
            "protocolo": "protocolo",
            "data_analise": "data_analise",
            "agente": "usuario",
            "descricao": "descricao_irregularidades",
            "irregularidade": "descricao_irregularidades",
            "status_irregularidade": "etapa_falha",
            "auditor": "auditor",
            "situacao": "status",
...
def _apply_consulta_column_filters(queryset, column_filters: dict, *, use_tratados: bool):
    field_map = _consulta_column_db_map(use_tratados)
    for key, values in (column_filters or {}).items():
        if not values:
            continue
...
def _apply_consulta_sort(queryset, *, sort_key: str, sort_dir: str, use_tratados: bool):
    ...
    if not field:
        return queryset
    prefix = "-" if (sort_dir or "asc").lower() == "desc" else ""
    return queryset.order_by(f"{prefix}{field}", "-id")


def _consulta_filter_options(queryset, column_key: str, *, use_tratados: bool) -> list[str]:
    field_map = _consulta_column_db_map(use_tratados)
