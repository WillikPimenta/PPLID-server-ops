...
from apps.access.registry import QUAL_AUDITORIA_COMPLIANCE_ASSIGN, QUAL_AUDITORIA_COMPLIANCE_CREATE, QUAL_AUDITORIA_COMPLIANCE_VIEW
from apps.auditoria.models import QualidadePendenteAuditoriaCompliance
from apps.auditoria.services.auditoria_compliance_atividade_import import list_reinspecao_falhas
...
class AuditoriaComplianceFilaContextMixin:
    """Marca as views da fila física exclusiva de Auditoria Compliance."""
...
class AuditoriaComplianceFalhasView(AuditoriaComplianceFilaContextMixin, APIView):
    permission_classes = [IsAuthenticated, HasPortalPermission]
    portal_permission = QUAL_AUDITORIA_COMPLIANCE_VIEW
...
class AuditoriaComplianceFalhasBulkDeleteView(AuditoriaComplianceFilaContextMixin, APIView):
    permission_classes = [IsAuthenticated, HasPortalPermission]
    portal_permission = QUAL_AUDITORIA_COMPLIANCE_CREATE

    def post(self, request):
        from apps.auditoria.services.auditoria_compliance_atividade_import import bulk_delete_reinspecao_falhas
...
class AuditoriaComplianceFalhaDetailView(AuditoriaComplianceFilaContextMixin, APIView):
    permission_classes = [IsAuthenticated, HasPortalPermission]
    portal_permission = QUAL_AUDITORIA_COMPLIANCE_VIEW

    def initial(self, request, *args, **kwargs):
        if request.method in ("PATCH", "DELETE"):
            self.portal_permission = QUAL_AUDITORIA_COMPLIANCE_CREATE
        else:
            self.portal_permission = QUAL_AUDITORIA_COMPLIANCE_VIEW
        super().initial(request, *args, **kwargs)

    def _get_falha(self, pk: int) -> QualidadePendenteAuditoriaCompliance | None:
        return (
    ...
    def delete(self, request, pk: int):
        from apps.auditoria.services.auditoria_compliance_atividade_import import is_falha_realizada
...
class AuditoriaCompliancePresenceMeView(AuditoriaComplianceFilaContextMixin, APIView):
    permission_classes = [IsAuthenticated, HasPortalPermission]
    portal_permission = QUAL_AUDITORIA_COMPLIANCE_VIEW
...
class AuditoriaCompliancePresenceAdminView(AuditoriaComplianceFilaContextMixin, APIView):
    permission_classes = [IsAuthenticated, HasPortalPermission]
    portal_permission = QUAL_AUDITORIA_COMPLIANCE_ASSIGN
...
class AuditoriaComplianceDistribuirView(AuditoriaComplianceFilaContextMixin, APIView):
    permission_classes = [IsAuthenticated, HasPortalPermission]
    portal_permission = QUAL_AUDITORIA_COMPLIANCE_ASSIGN
...
class AuditoriaComplianceDirecionarView(AuditoriaComplianceFilaContextMixin, APIView):
    permission_classes = [IsAuthenticated, HasPortalPermission]
    portal_permission = QUAL_AUDITORIA_COMPLIANCE_ASSIGN
...
class AuditoriaComplianceMinhaFilaView(AuditoriaComplianceFilaContextMixin, APIView):
    permission_classes = [IsAuthenticated, HasPortalPermission]
    portal_permission = QUAL_AUDITORIA_COMPLIANCE_VIEW
...
class AuditoriaComplianceRemoverDirecionadoView(AuditoriaComplianceFilaContextMixin, APIView):
    permission_classes = [IsAuthenticated, HasPortalPermission]
    portal_permission = QUAL_AUDITORIA_COMPLIANCE_CREATE
...
class AuditoriaComplianceClaimView(AuditoriaComplianceFilaContextMixin, APIView):
...
class AuditoriaComplianceAuditorFilaView(AuditoriaComplianceFilaContextMixin, APIView):
    permission_classes = [IsAuthenticated, HasPortalPermission]
    portal_permission = QUAL_AUDITORIA_COMPLIANCE_ASSIGN
...
class AuditoriaComplianceRemoverFilaControleView(AuditoriaComplianceFilaContextMixin, APIView):
    permission_classes = [IsAuthenticated, HasPortalPermission]
    portal_permission = QUAL_AUDITORIA_COMPLIANCE_ASSIGN
...
class AuditoriaComplianceIniciarAnaliseView(AuditoriaComplianceFilaContextMixin, APIView):
    permission_classes = [IsAuthenticated, HasPortalPermission]
    portal_permission = QUAL_AUDITORIA_COMPLIANCE_CREATE

    def post(self, request, pk: int):
        allow = user_has_permission(request.user, QUAL_AUDITORIA_COMPLIANCE_ASSIGN)
        try:
...
class AuditoriaComplianceConcluirAnaliseView(AuditoriaComplianceFilaContextMixin, APIView):
    permission_classes = [IsAuthenticated, HasPortalPermission]
    portal_permission = QUAL_AUDITORIA_COMPLIANCE_CREATE
...
class AuditoriaComplianceControleAuditoresView(AuditoriaComplianceFilaContextMixin, APIView):
    permission_classes = [IsAuthenticated, HasPortalPermission]
    portal_permission = QUAL_AUDITORIA_COMPLIANCE_VIEW
...
class AuditoriaComplianceRedistribuirFilaView(AuditoriaComplianceFilaContextMixin, APIView):
    permission_classes = [IsAuthenticated, HasPortalPermission]
    portal_permission = QUAL_AUDITORIA_COMPLIANCE_ASSIGN
...
class AuditoriaComplianceHistoricoView(AuditoriaComplianceFilaContextMixin, APIView):
    permission_classes = [IsAuthenticated, HasPortalPermission]
    portal_permission = QUAL_AUDITORIA_COMPLIANCE_VIEW
