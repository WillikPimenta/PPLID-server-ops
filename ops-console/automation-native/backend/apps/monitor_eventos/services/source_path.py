...
FILE_PATTERN = "monitor-eventos-tratado_*.parquet"
DATE_IN_NAME = re.compile(r"monitor-eventos-tratado_(\d{4}-\d{2}-\d{2})\.parquet$", re.I)
...
def get_source_dir(override: str | None = None) -> Path:
    ...
    configured = (getattr(settings, "MONITOR_EVENTOS_SOURCE_DIR", None) or "").strip()
    if configured:
        return Path(configured)
    env_path = (os.environ.get("MONITOR_EVENTOS_SOURCE_DIR") or "").strip()
    if env_path:
        return Path(env_path)
    return Path(
        r"C:\Users\c93123a\OneDrive - EXPERIAN SERVICES CORP"
        r"\Planejamento - IDF - Bases\Bots\monitor-eventos-tratado"
    )
